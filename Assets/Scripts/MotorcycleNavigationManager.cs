...
namespace MotorcycleNavigation
{
    public sealed class MotorcycleNavigationManager : MonoBehaviour
    {
        ...
        private bool ShouldReplan()
        {
            ...
            closestPathIndex = FindClosestPathIndex(CurrentPlan.path, currentPose.position);
            float distToPath = Vector3.Distance(currentPose.position, CurrentPlan.path[closestPathIndex].position);
            if (distToPath > runtime.severeReplanDistanceFromPathMeters &&
                Time.time - lastPlanTime >= runtime.minReplanIntervalSeconds)
            {
                return true;
            }
